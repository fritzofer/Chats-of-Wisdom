...
function enqueue_chat_page_assets() {
    if (is_page_template('chat-page.php')) {
        // Enqueue chat.js
        wp_enqueue_script('chat-page-script', get_stylesheet_directory_uri() . '/chat.js', array(), null);
        
        // Localize script to pass configuration values to JavaScript
        wp_localize_script('chat-page-script', 'chatConfig', array(
            'apiUrl' => 'https://kvyh6yra9h.execute-api.us-east-1.amazonaws.com/dev/v1/'
        ));
        
        // Enqueue chat.css
        wp_enqueue_style('chat-page-style', get_stylesheet_directory_uri() . '/chat.css', array(), null);
    }
}
add_action('wp_enqueue_scripts', 'enqueue_chat_page_assets');
